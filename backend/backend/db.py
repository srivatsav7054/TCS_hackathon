...
def save_claim(claim_data: dict, raw_text: str = ""):
    ...
    cursor.execute(
        ...
        (claim_id, claim_type, claim_amount, policy_limit, incident_date,
         ...
         sla_hours, hours_remaining, previous_claims, raw_text, document_urls, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
            claim_data["claim_id"],
            ...
            raw_text,
            json.dumps(claim_data.get("document_urls", {})),
...
def get_all_claims() -> list[dict]:
    ...
    for row in rows:
        ...
        for field in ["submitted_documents", "required_documents", "risk_factors",
                       "missing_documents", "comparable_cases", "next_best_action", "document_urls"]:
...
def get_claim_summary(claim_id: str) -> dict | None:
    ...
    cursor.execute("""
        SELECT c.claim_id, c.claim_type, c.claim_amount, c.incident_date,
               ...
               c.submitted_documents, c.required_documents, c.document_urls
        FROM claims c
        LEFT JOIN triage_results t ON c.claim_id = t.claim_id
        WHERE c.claim_id = ?
    """, (claim_id,))
    ...
    document_urls = {}
    if d.get("document_urls") and isinstance(d["document_urls"], str):
        try:
            document_urls = json.loads(d["document_urls"])
        except json.JSONDecodeError:
            pass
